<!-- new line added by dhurba -->
<script>
$(document).ready(function()
{
    $('#CalenderCprNtwt').change(function(){
        var total = $('#CalenderCprTotal').val()
        var netwt = $('#CalenderCprNtwt').val();
        var total = parseFloat(total.replace(',',''));
        if(total<netwt)
        {
            $('#submit_btn').removeClass('primary');
            $('#submit_btn').addClass('disabled');
            alert('ERROR:: NTWT should be lesser than Total. Please Re-check and resubmit');
        }else{
...
<div class="calenderCprs form">
    ...
    <?php echo $this->Form->end(__('Submit'),['id'=>'submit_btn']); ?>
